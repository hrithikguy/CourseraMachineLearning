...
bestvalues = [0 0];
bestpredictionerror = 1000;
for i=1:length(C_vector)
    for j=1:length(sigma_vector)
        currentmodel = svmTrain(X, y, C_vector(i), @(x1, x2) gaussianKernel(x1, x2, sigma_vector(j)));
        predictions = svmPredict(currentmodel, Xval);
        predictionerror = mean(double(predictions ~= yval));
        if (predictionerror <= bestpredictionerror)
            bestpredictionerror = predictionerror;
            bestvalues(1) = C_vector(i);
            bestvalues(2) = sigma_vector(j);
        end
    end
end
